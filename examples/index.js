...
var ourid = 'U-' + Math.floor(Math.random() * 10000);
  ...
  const createGt = (ip) => {
    ...
    gt.on('joined', (room, roomState, users) => {
      ...
      rootElement.on('mousemove', (e) => {
        gt.updateUser({ 'x': e.pageX - topOffsetX, y: e.pageY - topOffsetY });
      })
    })
...
function createCursor(mountID) {
  if (ourid == mountID) return;

  const  colorScale = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"],
  // Assign a random color between 0 to 9
  colorHash = colorScale[Math.floor(Math.random()*10)];
  ...
  let containerDiv = $('<div/>', { id: mountID, 'class': 'cursor-container' }).appendTo('body');
